...
function fun_pso(d,part_n,l,u,max_iter)
    ...
    obj_func = [funcion_prueba2(x[i,:],d) for i=1:part_n] # Evaluación función objetivo
    ...
    for t = 1:max_iter
        ...
        for i = 1:part_n
            ...
            nva_obj_func[i] = funcion_prueba2(x[i,:],d)
...
if rank != 0
  ...
  for i = 1:size-1
    ...
    minimo_act = min(funcion_prueba2(min_r, d), funcion_prueba2(mssgrv, d))

    if minimo_act == funcion_prueba2(min_r, d)
      global min_r = min_r
    else
      global min_r = mssgrv
    end
